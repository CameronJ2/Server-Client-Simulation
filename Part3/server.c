...
int main() {
   ...
   //char message[1024] = {0};
   int placeInMessageQueue = 0;
   char messageQueue[500][4][1024] = {0};
   ...
   while (1){ //infinite loop
      ...
      int token_count = 0;
      while (token != NULL && token_count < 5){ //the largest set of data received from 5 points, separated by commas, so token count shouldn't be greater than 5
      ...
      // input is received and parsed, now to run the logic for what we do with the information.
      ...
      else if (strcmp(receivedInfo[1], "5") == 0){ //sysCall message send
         token = strtok(receivedInfo[2], "-"); //take first token (pid of client)
         int token_count = 0;
         while (token != NULL && token_count < 3){ //the largest set of data received from 5 points, separated by commas, so token count shouldn't be greater than 5
            strcpy(messageQueue[placeInMessageQueue][token_count], token);
            messageQueue[placeInMessageQueue][token_count][1024 - 1] = '\0'; // Null-terminate the string
            // Get the next token
            token = strtok(NULL, "-");
            token_count++;
         }
         for (int i = 0; i < 3; i++) {
            stripSpaces(messageQueue[placeInMessageQueue][i]);
         }

         printf("Processed message:\ntype: %s\nlength: %s\nmessage: %s", messageQueue[placeInMessageQueue][0], messageQueue[placeInMessageQueue][1], messageQueue[placeInMessageQueue][2]);
         placeInMessageQueue++;

      }

      else if (strcmp(receivedInfo[1], "6") == 0){ //sysCall message receive
         char typeRequested[1024];
         char length[10];
         char value[1024] = {0};
         char returnMessage[1024];
         strcpy(typeRequested, receivedInfo[2]);

         for (int i = 0; i <= placeInMessageQueue; i++){
            if (strcmp(typeRequested, messageQueue[i][0]) == 0){
               strcpy(length, messageQueue[i][1]);
               strcpy(value, messageQueue[i][2]);
               memset(messageQueue[i], 0, sizeof(messageQueue[i]));
               break;
            }
         }
         sprintf(returnMessage, "length: %s\nmessage: %s", length, value);
         char clientNumber[1024];
         strcpy(clientNumber, receivedInfo[0]);
         int indexOfClient = clientIndexFinder(clientNumber, clientPID);
         if (strcmp(value, "") == 0)
         {
            write(fdClient[indexOfClient], "No message of that type was found", strlen("No message of that type was found")+1);
         }
         else{
            write(fdClient[indexOfClient], returnMessage, strlen(returnMessage)+1);
         }
